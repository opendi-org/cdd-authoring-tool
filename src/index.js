...
const runtimeGraphData = initializeGraph(graphData, paper, graph);
let graphElements = runtimeGraphData.graphElements;
let graphLinks = runtimeGraphData.graphLinks;
let functionButtons = runtimeGraphData.functionButtons;

function handleJSONEditorChange(updatedContent, previousContent, { contentErrors, patchResult }) {
    const newRuntimeGraphData = initializeGraph(updatedContent.json, paper, graph);
    content = updatedContent;
    graphElements = newRuntimeGraphData.graphElements;
    graphLinks = newRuntimeGraphData.graphLinks;
    functionButtons = runtimeGraphData.functionButtons;
}

function handleGraphChange(graphElements, graphLinks, content)
{
    content.json = fileIO.saveGraphJSON(content.json, graphElements, graphLinks);
}
