...
/**
 * Double click canvas background (non-interactive elements):
 * Save the updated graph JSON.
 */
paper.on('element:pointerdblclick', function (cell) {
    //Some elements are tagged with nonInteractie attribute, meaning
    //they won't have another double click event functionality
    if(cell.model.get('nonInteractive'))
    {
        fileIO.saveGraphJSON(graphData, graphRects, graphLinks);
    }
});
...
function onRectDrag(cell) {
    if(cell.isElement())
    {
        //Use the cell's model field for 'id' to find it in our runtime dict
        const rect = graphRects[cell.get('id')];
        if(rect !== null)
        {
...
function addRectToGraph(rectJSON, graph, rectWidth = defaultRectWidth, rectHeight = defaultRectHeight)
{
     ...
     * id (string) - This element's UID. Used for storage/lookup in runtime rect dict
