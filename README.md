...
2. From the repository's base directory, get the required json schema by running these commands:
    1. `git submodule init`
    2. `git submodule update -r`
3. From repository's base directory, run `npm i` in a command prompt to install required node dependencies
4. In the same directory, run `npm start` to run webpack, initialize the dev server, and open the main project in a browser
5. If the previous step does not open your browser automatically, open a browser and navigate to http://localhost:80/cdd-authoring-tool/
